...
| **`--surface-strong` se queda en `#ffffff` (T5-04)** | Decidido el 2026-09-05. La ficha de *Glassmorphism* lista los blancos puros como anti-patrón, pero eso habla del cristal y este token no lo es: sus cuatro usuarios —enlace de saltar, campo y botón secundario del acceso, panel del límite de errores— son superficies opacas a propósito, y **ninguna lleva `backdrop-filter`**. El cristal está en otras nueve reglas que fijan su propio alfa. Y cambiarlo no se vería: `--surface-primary` es `rgba(255,255,255,0.92)`, que sobre el arranque del degradado compone a **#fdfdfe**; las tarjetas ya se pintan a un 1 % del blanco puro, así que un «blanco roto» solo añadiría un segundo casi-blanco sin regla que diga cuándo usar cuál. Hay una prueba que fija la premisa —el día que una superficie esmerilada use este token, la decisión hay que rehacerla—. Lo que sí apareció al mirarlo es T5-07. |
